        ...
        <div class="row">
            <div class="col-xs-12 text-right">
                <div class="form-group">
                    <a class="btn btn-primary" href="<?php echo base_url(); ?>candidate/add"><i class="fa fa-plus"></i> Add New Candidate</a>
                </div>
            </div>
        </div>
        <div class="row">
                  ...
                  <table class="table table-hover"  id="feedbackTable">
                    ...
                    if(!empty($records))
                    {
                        foreach($records as $record)
                        {
                            ...
                            $this->db->select('*');
                            $this->db->from('party');
                            $this->db->where('id', $record->party);
                            $queryq11 = $this->db->get();
                            $queryqr1=$queryq11->row();
                            
                            $this->db->select('*');
                            $this->db->from('zone');
                            $this->db->where('id', $record->zone);
                    ...
                    <tr>
                       ...
                       <td><?php  echo @$record->id;  ?></td>
                        ...
                        <td><?php if(@$queryqr10->name!='') { echo @$queryqr10->name; } ?></td>
                        ...
                        <td class="text-center">
                            <a class="btn btn-sm btn-success"  style="    background-color: #00a65a !important; color:white" href="<?php echo base_url().'candidate/view/'.$record->id; ?>" title="Edit"><i class="fa fa-eye"></i></a>

                            <a class="btn btn-sm btn-info" href="<?php echo base_url().'candidate/edit/'.$record->id; ?>" title="Edit"><i class="fa fa-pencil"></i></a>
...
<script type="text/javascript">
    ...
    if (confirm("Are you sure you want to delete this candidate?")) {
        $.ajax({
            url: '<?php echo base_url(); ?>candidate/delete', // URL of the delete method in the controller
            type: 'POST',
            data: {
                id: emojiId,
            },
...
function updatestatus(a,b){
    ...
    if (confirm("Are you sure you want to update this Option?")) {
        $.ajax({
            url: '<?php echo base_url(); ?>candidate/update', // URL of the delete method in the controller
            type: 'POST',
            data: {
                id: b,
                 status: a,
